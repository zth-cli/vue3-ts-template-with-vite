<template>
  <component
    ...
    v-model.trim="searchParam[search.key ?? handleProp(column.prop!)]"
    :data="search?.el === 'tree-select' ? columnEnum : []"
    :options="['cascader', 'select-v2'].includes(search?.el) ? columnEnum : []"
  ...
  >
    <template v-if="search.el === 'cascader'" #default="{ data }">
